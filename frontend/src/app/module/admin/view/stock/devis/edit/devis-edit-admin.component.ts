...
@Component({
  selector: 'app-devis-edit-admin',
  templateUrl: './devis-edit-admin.component.html'
})
export class DevisEditAdminComponent extends AbstractEditController<DevisDto, DevisCriteria, DevisAdminService>   implements OnInit {
    ...
    public prepareEdit() {
        this.item.dateDevis = this.devisService.format(this.item.dateDevis);
    }



    public validateDevisItems(){
   ...
   public addDevisItems() {
       ...
       if (this.errorMessages.length === 0) {
            if(this.devisItemsElement.id == null){
                this.item.devisItems.push(this.devisItemsElement);
            }else{
                const index = this.item.devisItems.findIndex(e => e.id == this.devisItemsElement.id);
                this.item.devisItems[index] = this.devisItemsElement;
            }
    ...
    private validateDevisItemsProduit(){
    ...
    private validateDevisItemsQuantite(){
    ...
    private validateDevisItemsPrixUnitaireHt(){
    ...
    private validateDevisItemsPrixUnitaireTtc(){
    ...
    private validateDevisItemsRemise(){
    ...
    private validateDevisItemsMontantHt(){
    ...
    private validateDevisItemsMontantTtc(){
