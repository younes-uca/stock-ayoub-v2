...
@Component({
  selector: 'app-devis-item-edit-admin',
  templateUrl: './devis-item-edit-admin.component.html'
})
export class DevisItemEditAdminComponent extends AbstractEditController<DevisItemDto, DevisItemCriteria, DevisItemAdminService>   implements OnInit {
   ...
   public async openCreateDevis(devis: string) {
        const isPermistted = await this.roleService.isPermitted('Devis', 'edit');
        if (isPermistted) {
             this.devis = new DevisDto();
