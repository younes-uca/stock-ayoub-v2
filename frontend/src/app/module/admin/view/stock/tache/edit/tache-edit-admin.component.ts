...
export class TacheEditAdminComponent extends AbstractEditController<TacheDto, TacheCriteria, TacheAdminService>   implements OnInit {
    ...
    public prepareEdit() {
        this.item.dateDebut = this.tacheService.format(this.item.dateDebut);
        this.item.dateFin = this.tacheService.format(this.item.dateFin);
    }
