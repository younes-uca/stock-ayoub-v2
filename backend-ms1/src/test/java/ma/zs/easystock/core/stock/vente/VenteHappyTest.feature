Feature: Vente
  ...
  Background:
    ...
    * def objectSchema = read('VenteSchema.json')
  ...
  Scenario Outline: Find All items (204 & 200)
    ...
    * postBody.numeroFacture = uniqueId
    * path <paths>
    * header Authorization = 'Bearer ' + adminToken
    * request payload
    * method <method>
    ...
    * match res == { pass: true, message: null }
    ...
    Examples:
      ...
      | 200          | ''            | GET    | ["each response contains objectSchema "," response[0].numeroFacture == postBody.numeroFacture"] |
