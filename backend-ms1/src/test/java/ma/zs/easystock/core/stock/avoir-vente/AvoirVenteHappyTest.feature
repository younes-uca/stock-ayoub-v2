Feature: AvoirVente
  ...
  Background:
    ...
    * def objectSchema = read('AvoirVenteSchema.json')
  ...
  Scenario Outline: Find All items (204 & 200)
    ...
    * postBody.numeroFactureAvoirVente = uniqueId
    * path <paths>
    * header Authorization = 'Bearer ' + adminToken
    * request payload
    * method <method>
    ...
    * match res == { pass: true, message: null }
    ...
    Examples:
      ...
      | 200          | ''            | GET    | ["each response contains objectSchema "," response[0].numeroFactureAvoirVente == postBody.numeroFactureAvoirVente"] |
